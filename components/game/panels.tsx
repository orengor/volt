    ...
    <header className="pointer-events-none absolute inset-x-0 top-0 z-20 p-2 pr-[min(46vw,268px)] pt-[max(0.45rem,env(safe-area-inset-top))] sm:pr-[276px]">
      <div className="pointer-events-auto mx-auto flex max-w-[720px] items-center gap-2 rounded-2xl border border-[#2ee6c7]/35 bg-[#141c24]/88 px-2 py-1.5 text-white shadow-[0_10px_28px_rgba(12,18,24,0.35)] backdrop-blur-md md:px-3">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button size="icon" variant="ghost" className="size-11 text-white hover:bg-white/10" onClick={onShop} aria-label="Shop">
...
export function DispatchColumn({
  ...
  const live = snap.orders.filter((o) => o.status !== "late");
              ...
              live.map((o) => (
                <button
                  ...
                  className={cn(
                    ...
                    o.driverId ? "border-transparent bg-[#1e2a34]" : "border-dashed border-[#ffd166]/70 bg-[#1a242c]",
                ...
                >
                  ...
                  <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/10">
                    <div
                      className={cn("h-full rounded-full", o.cookReady ? "bg-[#2ee6c7]" : "bg-[#ffd166]")}
                      style={{ width: `${Math.round(o.cookPct * 100)}%` }}
                    />
                  </div>
                  <div className="mt-1.5 flex items-center gap-1.5 pl-6">
                    ...
                    <span className="ml-auto shrink-0 text-[10px] text-[#9aa3ab]">{o.dueLabel.replace(" — slipping", "")}</span>
                  </div>
                </button>
              ))
            )}
            ...
            {snap.drivers.map((d) => (
              <button
                ...
                onClick={(e) => {
                  ...
                  className={cn(
                    "flex min-h-11 w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left",
                    d.selected ? "bg-[#1e2a34] ring-2 ring-[#2ee6c7]" : "hover:bg-white/5"
                  )}
              >
                <span className="relative size-3.5 shrink-0 rounded-full ring-2 ring-white/30" style={{ background: d.color }}>
                  {d.jobColor ? (
                    <span className="absolute -right-0.5 -bottom-0.5 size-2 rounded-full border border-[#0f161c]" style={{ background: d.jobColor }} />
                  ) : null}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-heading text-[13px] font-semibold">
                    {d.name}
                    <span className="ml-1 font-heading text-[10px] uppercase tracking-wide text-[#2ee6c7]">{d.jobVerb}</span>
                  </span>
                  <span className="block truncate text-[10px] text-[#9aa3ab]">{d.statusLine}</span>
                </span>
